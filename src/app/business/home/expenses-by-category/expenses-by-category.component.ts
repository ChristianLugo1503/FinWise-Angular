...
export default class ExpensesByCategoryComponent implements OnInit {
      ...
      if (data !== null) {
        ...
        this.categoriesSrv
          ...
          .subscribe((allCategories: any[]) => {
            this.categoriesWithImagesAndAmounts = this.categoriesData.map(
              (categoryName, index) => {
            ...
            this.categoriesWithImagesAndAmounts.sort(
  ...
  openCustomModal(type: string, img: string, transactionsID: number) {
    this.modalSrv.openModal(type, img, transactionsID);
  }
}
