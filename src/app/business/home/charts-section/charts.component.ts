  ...
  filterDay(): void {
    ...
    if (this.selectedDay) {
      this.transactionsSrv.getTransactionsData().subscribe({
        ...
        next: (response) => {
          if (response) {
            const { categoriesData, amountsData, transactionID } =
              response.reduce(
                (acc, transaction: any) => {
                  const normalizedDbDate = new Date(transaction.date)
                    .toISOString()
                    .split('T')[0]; // Se normaliza la fecha de la bd
            ...
            const categorySums = categoriesData.reduce(
            ...
            this.series = this.categories.map(
      ...
      this.transactionsSrv.getTransactionsData().subscribe({
        ...
        next: (response) => {
          if (response) {
            ...
            const { categoriesData, amountsData, datesData, transactionID } =
              response.reduce(
                (acc, transaction: any) => {
                  const normalizedDbDate = new Date(transaction.date)
                    .toISOString()
                  ...
                  if (transaction.type !== this.activeTab) return acc; // Se filtra por categoría
                  if (
                    normalizedDbDate < this.selectedRange!.start ||
                    normalizedDbDate > this.selectedRange!.end
                  )
                    return acc; //filtrar por el rango de fechas
                  acc.categoriesData.push(transaction.categoryID.name);
                  acc.amountsData.push(transaction.amount);
            ...
            const categorySumsDate = categoriesData.reduce(
              (acc: any, category: any, index: any) => {
            ...
            const categorySumsArray = Object.entries(categorySumsDate).map(
              ([key, amount]) => {
            ...
            this.series = categorySumsArray.reduce(
              (acc: any, { category, date, amount }) => {
                // Verificar si la categoría ya existe en el acumulador
                let categoryEntry = acc.find(
                ...
                if (!categoryEntry) {
                  ...
                  categoryEntry = {
                    name: category,
                    data: new Array(this.dates.length).fill(0),
                  };
      ...
      this.transactionsSrv.getTransactionsData().subscribe({
        next: (response) => {
          if (response) {
            ...
            const categorySumsMonth = categoriesData.reduce(
              (acc: any, category: any, index: any) => {
            ...
            // Convertir el objeto en un arreglo de objetos
            const categorySumsArrayMonth = Object.entries(
              categorySumsMonth
            ).map(([key, amount]) => {
            ...
            this.series = this.categories.map((category) => {
              ...
              categorySumsArrayMonth.forEach(
                ({ category: cat, month, amount }) => {
                  if (cat === category) {
                    data[month] = amount;
                  }
                }
